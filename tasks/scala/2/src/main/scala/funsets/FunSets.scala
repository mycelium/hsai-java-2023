...
object FunSets {
   ...
   */
    def diff(s: Set, t: Set): Set = (num: Int) => contains(s, num) && !contains(t, num)
  
  /**
   * Returns the subset of `s` for which `p` holds.
   */
    def filter(s: Set, p: Int => Boolean): Set = (num: Int) => contains(s, num) && p(num)
   ...
   */
    def map(s: Set, f: Int => Int): Set = {
      ...
      iter(first(-bound), singletonSet(f(first(-bound))))
    }
